...
public class GoapPlanner : IGoapPlanner
{
    ...
    bool FindPath(Node parent, HashSet<AgentAction> actions)
    {
        ...
        foreach (var action in orderedActions)
        {
            ...
            Debug.Log($"Checking action: {action.Name}, cost: {action.Cost}");
            foreach (var effect in action.Effects)
            {
                Debug.Log($"Action {action.Name} has effects: {effect.Name}");
            }

            var requiredEffects = parent.RequiredEffects;
            foreach (var effect in requiredEffects)
            {
                Debug.Log($"We require an action with effects: {effect.Name}");
            }
